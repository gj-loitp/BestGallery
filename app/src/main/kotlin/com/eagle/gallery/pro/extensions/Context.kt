...
fun Context.launchSettings() {
    startActivity(Intent(applicationContext, com.eagle.gallery.pro.activities.SettingsActivity::class.java))
}
...
fun Context.rescanFolderMediaSync(path: String) {
    getCachedMedia(path) {
        val cached = it
        com.eagle.gallery.pro.asynctasks.GetMediaAsynctask(applicationContext, path, false, false, false) {
            Thread {
                val newMedia = it
