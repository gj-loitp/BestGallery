...
import com.roy.commons.ext.areDigitsOnly
...
import com.roy.commons.helpers.photoExtensions
import com.roy.commons.helpers.rawExtensions
import com.roy.commons.helpers.videoExtensions
import java.io.File
import java.util.*
...
class MediaFetcher(val context: Context) {
    ...
    fun getFoldersToScan(): ArrayList<String> {
        ...
        val selection =
            "${getSelectionQuery(filterMedia)} ${MediaStore.Images.ImageColumns.BUCKET_ID} IS NOT NULL) GROUP BY (${MediaStore.Images.ImageColumns.BUCKET_ID}"
    ...
    private fun parseCursor(cursor: Cursor): ArrayList<String> {
        ...
        var foldersToScan =
            config.everShownFolders.filter { it == FAVORITES || it == RECYCLE_BIN || File(it).exists() }
        ...
        foldersToScan = foldersToScan.filter {
    ...
    private fun getMediaInFolder(
        ...
        val showHidden = config.shouldShowHidden
        val dateTakens =
            if (getProperDateTaken && folder != FAVORITES && !isRecycleBin) getFolderDateTakens(
                folder
            ) else HashMap()
    ...
    fun groupMedia(media: ArrayList<Medium>, path: String): ArrayList<ThumbnailItem> {
        ...
        val sorted =
            if (currentGrouping and GROUP_BY_DATE_TAKEN != 0 || currentGrouping and GROUP_BY_LAST_MODIFIED != 0) {
    ...
    private fun getFormattedKey(
        ...
        return when {
            grouping and GROUP_BY_LAST_MODIFIED != 0 || grouping and GROUP_BY_DATE_TAKEN != 0 -> getFinalDate(
                formatDate(key),
                today,
                yesterday
            )
