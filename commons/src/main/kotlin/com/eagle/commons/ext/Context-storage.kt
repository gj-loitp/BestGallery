...
import com.eagle.commons.helpers.isMarshmallowPlus
...
fun Context.getDocumentFile(path: String): DocumentFile? {
    ...
    val uri: Uri?
    try {
        uri = Uri.parse(if (isOTG) baseConfig.OTGTreeUri else baseConfig.treeUri)
    } catch (e: Exception) {
        e.printStackTrace()
        return null
    }
    ...
    try {
        document = DocumentFile.fromTreeUri(applicationContext, uri)
    } catch (e: Exception) {
        e.printStackTrace()
        return null
    }
